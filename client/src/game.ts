...
text.addComponent(
  new Transform({
    position: new Vector3(1.4 - 0.05, 1.5, 3 - 0.05),
...
let updateText = () => {
  ...
  textMsg.fontSize = 10
  text.addComponentOrReplace(textMsg)
}
...
const slot = new Slot_Machine(new Vector3(3, 0, 2), socket)
slot.show()
slot.rotate(135)
...
board.addComponent(collideBox)
engine.addEntity(board)

board.getComponent(Transform).rotation.eulerAngles = new Vector3(0, 45, 0)
text.getComponent(Transform).rotation.eulerAngles = new Vector3(0, 180+45, 0)
